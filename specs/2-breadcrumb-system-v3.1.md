...
8. **Atomic writes protect data.** *(New in v3.1)* All writes to critical index and recipe files use an atomic write protocol (temp file → move) to prevent corruption from interrupted writes.
...
├── index.json.bak                      ← backup of previous index (v3.1: Atomic Write Protocol)
├── recipe.json                         ← capabilities manifest (the Bread Recipe)
├── recipe.json.bak                     ← backup of previous recipe (v3.1: Atomic Write Protocol)
...
Orphan breadcrumbs — those residing in `~/breadcrumb-trail/orphans/` — follow a managed lifecycle to prevent stale documentation from accumulating indefinitely.

#### The Problem

When the Baker auto-drafts breadcrumbs for discovered components (based on README.md, SKILL.md, or `--help` output), these drafts are placed in `orphans/` with `status: experimental`. Without active validation, these drafts can linger forever — cluttering the index and eroding trust in the system.
...
```
PHASE 2 — VALIDATE (Orphan Lifecycle)
  Orphan count: 23 / 50 (max_orphan_count)
  Expiring soon (< 7 days): 3
  Expired this run: 1 → moved to archive/
  Pruned for cap: 0
```
...
If the Baker writes directly to `index.json` and the process is interrupted mid-write, the file is left in a corrupt state — broken JSON that no agent can parse. The entire Breadcrumb System becomes non-functional until the file is manually repaired.

#### Protocol Steps

When the Baker writes `index.json` or `recipe.json`:

1. **Backup** — Copy the current file to a `.bak` backup before any modification:
   ```bash
...
2. **Write to temp** — Write the new content to a temporary file in the same directory:
   ```bash
   # Write new index to temp file
   echo "$NEW_INDEX_JSON" > ~/breadcrumb-trail/index.json.tmp
   ```

3. **Validate temp** — Verify the temp file is valid JSON before proceeding:
   ```bash
   jq empty ~/breadcrumb-trail/index.json.tmp
   ```

4. **Atomic move** — Only replace the final file on successful validation:
   ```bash
   mv ~/breadcrumb-trail/index.json.tmp ~/breadcrumb-trail/index.json
   ```

5. **Cleanup on failure** — If validation fails, remove the temp file and log the error:
   ```bash
   rm -f ~/breadcrumb-trail/index.json.tmp
   echo "ERROR: index.json write failed validation — kept previous version" >> baker.log
   ```
...
1. Log the corruption: `"CRITICAL: {file} is corrupt — falling back to .bak"`
...
def atomic_write(filepath, new_content):
    ...
    try:
        ...
        os.remove(tmp_path)
        log("ERROR: atomic write validation failed — kept previous version")
        return False
...
- **Atomic Write Protocol:** All writes to index.json and recipe.json MUST use
  atomic writes (write to .tmp → validate → mv to final). Always create .bak
  backups before overwriting. See §9.1 of the spec. Never write directly to
  these files — corruption from interrupted writes breaks the entire system.
...
- **Orphan Lifecycle:** Auto-drafted orphan breadcrumbs expire after 30 days if
  ...
  first, then oldest experimental orphans. See §8.1 of the spec.
...
- status: experimental
- draft_expires: {created date + 30 days}  ← (v3.1: Orphan Lifecycle)
...
6. For each orphan in ~/breadcrumb-trail/orphans/:
   a. If `status: experimental` AND `draft_expires` date has passed:
      → Move to ~/breadcrumb-trail/archive/
      → Set status: deprecated
      → Add changelog: "Auto-expired: draft not validated within 30 days"
      → Log in baker.log
   b. Count total orphans. If count > max_orphan_count (default: 50):
      → Prune oldest expired orphans first
      → Then oldest experimental orphans
      → Until count ≤ max_orphan_count
      → Log all pruned orphans in baker.log
...
**All writes in this phase MUST use the Atomic Write Protocol (§9.1).**
...
3. Rebuild ~/breadcrumb-trail/index.json using atomic write:
   a. Backup: cp index.json → index.json.bak
   b. Write to index.json.tmp
   c. Validate with jq
   d. mv index.json.tmp → index.json
...
5. Validate ~/breadcrumb-trail/recipe.json using atomic write:
   a. Backup: cp recipe.json → recipe.json.bak
   b. Write updates to recipe.json.tmp
   c. Validate with jq
   d. mv recipe.json.tmp → recipe.json
...
Append to ~/breadcrumb-trail/baker.log on every run:
  ...
  PHASE 2 — VALIDATE
    ...
    - Expired this run: {N} → moved to archive/
    - Pruned for cap: {N}
  PHASE 3 — INDEX
    Total breadcrumbs: {N}
    Active: {N} | Experimental: {N} | Deprecated: {N} | Broken: {N}
    Recipe capabilities: {N} active, {N} flagged
    Atomic writes: index.json ✓ | recipe.json ✓
    Backups: index.json.bak ✓ | recipe.json.bak ✓
  === END BAKER RUN ===
```
...
```
1. Agent reads index.json → JSON parse error
2. Agent reads index.json.bak → valid JSON, last-known-good state
3. Agent copies: cp index.json.bak index.json
4. Agent logs: "CRITICAL: index.json was corrupt — restored from .bak"
...
experimental → [30 days, no validation] → deprecated → archived (auto-expired)
```
...
- *(v3.1)* Auto-drafted orphans with `status: experimental` expire after 30 days (per §8.1) — separate from the 90-day deprecation window for manually-created breadcrumbs
...
2. **§8.1 — Orphan Lifecycle & 30-Day Draft Expiration**
...
- Breadcrumb Baker agent with 3-phase operation (Discover → Validate → Index)
